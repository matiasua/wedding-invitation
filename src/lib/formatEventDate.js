 ...
 * @returns {string} Fecha formateada en español para Santiago de Chile
 *
 * @example
...
export const formatEventDate = (isoString, format = 'full') => {
    ...
    if (format === 'time') {
        return date.toLocaleTimeString('es-CL', formats[format]);
    }

    return date.toLocaleDateString('es-CL', formats[format]);
